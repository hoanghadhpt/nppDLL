...
namespace phdesign.NppToolBucket
{
    public class FindAndReplace
    {
        ...
        private void OnDoAction(object sender, DoActionEventArgs doActionEventArgs)
        {
            ...
            switch (doActionEventArgs.Action)
            {
                ...
                case Forms.Action.ReplaceAll4:
                    ...
                    _searchScope = null;
                    break;
                case Forms.Action.AdvEmph:
                    AdvEmphasys();
                    _searchScope = null;
                    break;
                case Forms.Action.footnote2end:
                    Ftnt2end();
                    _searchScope = null;
                    break;
        ...
        private void AdvEmphasys()
        {
            ...
            foreach (var c in az)
            {
                ReplaceAll("."+ c.ToString() + "</emph>.", "." + c.ToString() + ".</emph>");
                ReplaceAll(" " + c.ToString() + "</emph>.", " " + c.ToString() + ".</emph>");
            }
        }
        ...
        internal void Ftnt2end()
        {
            ...
            foreach (Match m in Regex.Matches(text, pattern, options))
            {
                ...
                System.Windows.Forms.MessageBox.Show(m.Value.ToString());
            }
