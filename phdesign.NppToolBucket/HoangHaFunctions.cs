...
namespace phdesign.NppToolBucket
{
    internal static class HoangHaFunctions
    {
        ...
        // Chùm 252
        internal static void C252_VISF()
        {
            var editor = Editor.GetActive();
            var filename = editor.GetCurrentFileName();
            var Dir = editor.GetCurrentDirectory();
            ...
            {
                class252 c252 = new class252();
                text = c252.di252_PreProcess(text);
                editor.SetDocumentText(text);
            }
        }

        internal static void C252_XML()
        {
            var editor = Editor.GetActive();
            var filename = editor.GetCurrentFileName();
            var Dir = editor.GetCurrentDirectory();
        ...
        internal static void CheckDuplicateSegments()
        {
            var editor = Editor.GetActive();
            var text = editor.GetDocumentText();
            var filePath = editor.GetCurrentDirectory();
            var fileName = editor.GetCurrentFileName();
            if (String.IsNullOrEmpty(text))
            {
                return;
            ...
            {
                // Cleanup Reports
                File.Delete(filePath + "\\" + Path.GetFileNameWithoutExtension(fileName) + "_duplicated.segments");
                // Split Into Multiple Files
                string[] listFiles = Regex.Split(text, @"(?=\$00:.*?:)");
                int fileno = 0;
                foreach (string file in listFiles)
                {
                    ...
                    MatchCollection collection = Regex.Matches(file, @"(\$\d{2,3})\:");
                    foreach (Match c in collection)
                    {
                        Segments.Add(c.Groups[1].Value);
                    }

                    IEnumerable<string> duplicates = Segments.GroupBy(x => x)
                                        .SelectMany(g => g.Skip(1));
                    foreach (string dup in duplicates)
                    {
                        MessageBox.Show("Duplicated Segments: " + dup +"\r\nFile No.:\t"+fileno.ToString());
                        File.AppendAllText(filePath + "\\" + Path.GetFileNameWithoutExtension(fileName) + "_duplicated.segments", "File: " + fileno + " -- Segments: " + dup + "\r\n");
                    }
                    fileno++;
                }
            }
        ...
        internal static void QA_Dola52toDola40()
        {
            var editor = Editor.GetActive();
            var text = editor.GetDocumentText();
            if (String.IsNullOrEmpty(text))
            ...
            else
            {
                ...
                foreach (string file in listFiles)
                {
                    Match Mdola52 = Regex.Match(file, @"\$52:\$S (.*?)(\d+)\.? ");
                    string dola52 = Mdola52.Groups[1].Value + Mdola52.Groups[2].Value;
                    string f = Regex.Replace(file, @"\$40:(.*) \$S .*", "$40:$1 $S " + dola52);
                    allFiles += f;
                }
                editor.SetDocumentText(allFiles);
            }
        }
