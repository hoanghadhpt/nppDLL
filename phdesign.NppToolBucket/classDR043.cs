...
namespace phdesign.NppToolBucket
{
    class classDR043
    {
        public string fProcess(string input)
        {
            string output = input;
            // Font
            ...
            output = Regex.Replace(output, @"\$20:§ ?(.*?) (.*)", m => $"<lnv:CITE>_____cite_____ §{m.Groups[1].Value.Trim()}</lnv:CITE>\r\n");
            // <lnv:DOC-HEADING>
            ...
            output = output.Replace(@"$=S", "<lnvxe:blockquote>\r\n");
            output = output.Replace(@"  \r<", "<");
            output = output.Replace(@"</lnv:TEXT-1>\n", "</lnv:TEXT-1>\r\n");
            output = output.Replace(@"</lnv:NORMCITE>\n", "</lnv:NORMCITE>\r\n");

            // return
            return output;
        }

        public string fPreProcess(string input)
        {
            string output = input;

            output = Regex.Replace(input, @"\$20:§ ?(.*?) (.*)", m => $"$20:§ {m.Groups[1].Value.Trim()} {m.Groups[2].Value.Trim()}\r\n" +
            $"\r\n$200:_____cite_____ §{m.Groups[1].Value.Trim()}\r\n");
        ...
        public string fReNumberLevel(string input)
        {
        ...
        ///
        public string fLink(string input)
        {
